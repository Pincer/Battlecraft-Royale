    ...
    public boolean generate(World world, Random random, int i, int j, int k)
    {
        for(int l = 0; l < 16; l++)
        {
            ...
            if(world.isAirBlock(i1, j1, k1) && (world.getBlockId(i1, j1 - 1, k1) == Block.grass.blockID || world.getBlockId(i1, j1 - 1, k1) == Block.snow.blockID || world.getBlockId(i1, j1 - 1, k1) == Block.sand.blockID) && Block.chest.canPlaceBlockAt(world, i1, j1, k1))
            {
            	...
            	for(int x = 0 ; x < 5; x++){
            		for(int y = 0 ; y < 4; y++){
            			world.setBlockAndMetadata(i1+x, j1-1, k1+y, Block.planks.blockID, random.nextInt(4));
            		}
            	}
            	if(world.isAirBlock(i1-1, j1-1, k1+1)){
            		world.setBlockAndMetadata(i1-1, j1-1, k1+1, Block.stairCompactPlanks.blockID, random.nextInt(4));
            	}
                ...
                //Windows
                world.setBlockAndMetadata(i1+2, j1+1, k1, Block.glass.blockID, random.nextInt(4));
                world.setBlockAndMetadata(i1+2, j1+1, k1+3, Block.glass.blockID, random.nextInt(4));
